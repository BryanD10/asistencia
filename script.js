...
function generarClave(event) {
  ...
  // Teléfono y correo son OPCIONALES: solo se validan si el usuario
  // escribió algo. Si vienen vacíos, se aceptan tal cual.
  var telefonoTrim = (telefonoInput || "").trim();
  if (telefonoTrim !== "" && !validarTelefono(telefonoTrim)) {
    alertBox.textContent = "❌ El teléfono debe tener 8 dígitos válidos (Ej: 71234567), o deja el campo vacío.";
    alertBox.className = "alert-box error";
    alertBox.style.display = "block";
  ...
  var correoTrim = (correoInput || "").trim();
  if (correoTrim !== "" && !validarCorreo(correoTrim)) {
    alertBox.textContent = "❌ Ingresa un correo electrónico válido, o deja el campo vacío.";
    alertBox.className = "alert-box error";
    alertBox.style.display = "block";
  ...
  var telefono = limpiarTelefono(telefonoTrim);
  var correo = correoTrim.toLowerCase();
  ...
  firmarPayload("buscar_alumno", { nombre: nombre }, null)
    ...
    .then(function(data) {
      if (data.clave) {
        ...
        crearNuevaClave(nombre, docenteInput, telefono, correo, cumple, hpGen, alertBox, btn, containerClave);
      }
    })
    .catch(function() {
...
function crearNuevaClave(nombre, docente, telefono, correo, cumple, hp, alertBox, btn, containerClave) {
...
// ==========================================
// SUBIDA DE VIDEO DE RESPALDO (opcional) → Google Drive
// Flujo: <input type="file"> -> FileReader (Base64) -> fetch() firmado
// -> doPost("subir_video") -> Drive. Si falla, NO bloquea el registro:
// solo se muestra el error en el mismo recuadro de progreso.
// ==========================================
var VIDEO_MAX_BYTES = 15 * 1024 * 1024; // 15MB (antes de Base64)

function mostrarEstadoVideo(mensaje, tipo) {
  var box = document.getElementById("video-progress");
  if (!box) return;
  box.textContent = mensaje;
  box.className = "alert-box " + (tipo || "info");
  box.style.display = "block";
}

function subirVideoEvidencia(event) {
  var input = event.target;
  var archivo = input.files && input.files[0];
  if (!archivo) return;

  if (!archivo.type || archivo.type.indexOf("video/") !== 0) {
    mostrarEstadoVideo("❌ El archivo debe ser un video.", "error");
    input.value = "";
    return;
  }

  if (archivo.size > VIDEO_MAX_BYTES) {
    mostrarEstadoVideo("❌ El video pesa más de 15MB. Elige uno más liviano.", "error");
    input.value = "";
    return;
  }

  mostrarEstadoVideo("⏳ Leyendo el video...", "info");

  var lector = new FileReader();
  lector.onerror = function() {
    mostrarEstadoVideo("❌ No se pudo leer el archivo. Intenta de nuevo.", "error");
  };
  lector.onload = function() {
    // El resultado viene como "data:video/mp4;base64,AAAA..." — se
    // quita el prefijo para mandar solo los datos puros al backend.
    var resultado = lector.result || "";
    var partido = resultado.split(",");
    var base64Data = partido.length > 1 ? partido[1] : "";

    if (!base64Data) {
      mostrarEstadoVideo("❌ No se pudo procesar el video.", "error");
      return;
    }

    mostrarEstadoVideo("⏳ Subiendo video a Drive... esto puede tardar según tu conexión.", "info");

    var nombreArchivo = sanitizarInput(archivo.name || "video") || "video.mp4";
    var claveInput = sanitizarInput((document.getElementById("reg-key") || {}).value || "").toUpperCase();

    firmarPayload("subir_video", {
      ...
      .then(function(data) {
        if (data.result === "success") {
          mostrarEstadoVideo("✅ Video subido correctamente a Drive.", "success");
        } else {
          mostrarEstadoVideo("⚠️ " + (data.message || "No se pudo subir el video."), "warning");
        }
      })
      .catch(function() {
        mostrarEstadoVideo("❌ Error de red al subir el video. Tu asistencia no se ve afectada.", "error");
      });
  };
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  var videoField = document.getElementById("reg-video");
  if (videoField) {
    videoField.addEventListener("change", subirVideoEvidencia);
  }
